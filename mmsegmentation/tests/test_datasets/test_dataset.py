...
def test_classes():
    ...
    assert (
        list(PascalVOCDataset.METAINFO["classes"])
        == get_classes("voc")
        == get_classes("pascal_voc")
    )
    assert (
        list(ADE20KDataset.METAINFO["classes"])
        == get_classes("ade")
        == get_classes("ade20k")
    )
...
def test_palette():
    ...
    assert (
        PascalVOCDataset.METAINFO["palette"]
        == get_palette("voc")
        == get_palette("pascal_voc")
    )
    assert (
        ADE20KDataset.METAINFO["palette"] == get_palette("ade") == get_palette("ade20k")
    )
    ...
    test_dataset = CityscapesDataset(
        ...
        data_prefix=dict(
            ...
            seg_map_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_cityscapes_dataset/gtFine/val"
            ),
        ),
    )
...
def test_synapse():
    test_dataset = SynapseDataset(
        ...
        data_prefix=dict(
            img_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_synapse_dataset/img_dir"
            ),
            seg_map_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_synapse_dataset/ann_dir"
            ),
        ),
    )
...
def test_refuge():
    test_dataset = REFUGEDataset(
        ...
        data_prefix=dict(
            img_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_refuge_dataset/img_dir"
            ),
            seg_map_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_refuge_dataset/ann_dir"
            ),
        ),
    )
    assert len(test_dataset) == 1
...
def test_decathlon():
    ...
    test_dataset = DecathlonDataset(
        pipeline=[], data_root=data_root, ann_file="dataset.json", test_mode=True
    )
    assert len(test_dataset) == 3
...
def test_lip():
    ...
    train_dataset = LIPDataset(
        pipeline=[],
        data_root=data_root,
        data_prefix=dict(img_path="train_images", seg_map_path="train_segmentations"),
    )
    ...
    test_dataset = LIPDataset(
        pipeline=[],
        data_root=data_root,
        data_prefix=dict(img_path="val_images", seg_map_path="val_segmentations"),
    )
    assert len(test_dataset) == 1
...
def test_mapillary():
    test_dataset = MapillaryDataset_v1(
        ...
        data_prefix=dict(
            img_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_mapillary_dataset/images"
            ),
            seg_map_path=osp.join(
                osp.dirname(__file__), "../data/pseudo_mapillary_dataset/v1.2"
            ),
        ),
    )
    assert len(test_dataset) == 1
...
def test_custom_dataset_custom_palette():
    dataset = BaseSegDataset(
        ...
        ann_file=tempfile.mkdtemp(),
        metainfo=dict(
            classes=("bus", "car"), palette=[[100, 100, 100], [200, 200, 200]]
        ),
        lazy_init=True,
        test_mode=True,
    )
    assert tuple(dataset.metainfo["palette"]) == tuple(
