...
def test_resize():
    ...
    min_size_ratio = max(640 / img.shape[0], 640 / img.shape[1])
    transform = dict(
        type="Resize", scale=(2560, 640), scale_factor=min_size_ratio, keep_ratio=True
    )
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
...
def test_random_rotate_flip():
    ...
    assert (
        str(transform) == "{'type': 'RandomRotFlip',"
        " 'flip_prob': 0,"
        " 'rotate_prob': 1.0,"
        " 'degree': 20}"
    )
...
def test_biomedical_gaussian_noise():
    ...
    assert (
        str(noise_module) == "BioMedicalGaussianNoise"
        "(prob=1.0, "
        "mean=0.0, "
        "std=0.1)"
    )
...
def test_biomedical_gaussian_blur():
    ...
    transform = dict(
        type="BioMedicalGaussianBlur", prob=1.0, different_sigma_per_axis=False
    )
    smooth_module = TRANSFORMS.build(transform)
...
def test_BioMedicalRandomGamma():
    ...
    with pytest.raises(AssertionError):
        transform = dict(
            type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 0.2, 0.3)
        )
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(
            type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), invert_image=1
        )
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(
            type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), per_channel=1
        )
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(
            type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), retain_stats=1
        )
        TRANSFORMS.build(transform)
...
def test_biomedical_3d_flip():
    ...
    transform = dict(
        type="BioMedical3DRandomFlip", prob=1, axes=(1, 2), swap_label_pairs=[(0, 1)]
    )
    transform = TRANSFORMS.build(transform)
    results = data.copy()
    results = transform(results)
