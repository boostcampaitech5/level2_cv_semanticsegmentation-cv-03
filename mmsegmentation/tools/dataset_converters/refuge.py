...
def extract_img(
    root: str, cur_dir: str, out_dir: str, mode: str = "train", file_type: str = "img"
) -> None:
    """_summary_
    ...
    for cur_dir, dirs, files in os.walk(root):
        ...
        if (
            len(dirs) == 0
            and cur_dir.split("\\")[-1].find("Illustration") == -1
            and cur_dir.find("MACOSX") == -1
        ):
            file_names = [
                file for file in files if file.endswith(".jpg") or file.endswith(".bmp")
            ]
            for filename in sorted(file_names):
                ...
                mmcv.imwrite(
                    img,
                    osp.join(
                        out_dir, file_type, mode, osp.splitext(filename)[0] + ".png"
                    ),
                )
...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for file in files:
            ...
            file_type = (
                "images"
                if file.find("Anno") == -1 and file.find("GT") == -1
                else "annotations"
            )
